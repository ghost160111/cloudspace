...
export type T_Func = <T extends string>(path: T, vars?: Record<string, string>) => string;
...
class LocaleStore<T extends string> extends MobxStore implements IInitializable {
    ...
    init(): void {
        ...
        const onRouteChange = (): void => {
            const lang: string | null = getLanguageFromPath();
            if (lang === null) return;
            this.setLocale(lang);
        };
    ...
    *loadTranslation(lang: string): Generator<Promise<void>, void, any> {
    ...
    @bound
    t(path: T, vars?: Record<string, string>): string {
